...
def train_perceptron():
    ...
    # Create word-to-index mapping for numpy arrays
    word_to_idx = {word: idx for idx, word in enumerate(vocabulary)}
    feature_size = len(vocabulary) + 1  # +1 for bias term
    ...
    def extract_features(text):
        ...
        # Create feature vector (binary word presence) as numpy array
        features = np.zeros(feature_size)
        for word in words:
            if word in word_to_idx:
                features[word_to_idx[word]] = 1
                
        # Add bias term
        features[-1] = 1  # bias is the last element
        return features
    
    # Step 5: Initialize perceptron weights for each emotion using numpy
    weights = {}
    for emotion in emotions:
        weights[emotion] = np.zeros(feature_size)  # Initialize weights as numpy array
    ...
    for epoch in range(max_epochs):
        ...
        for emotion, text in train_data:
            ...
            # Calculate scores for each emotion using numpy dot product
            scores = {}
            for e in emotions:
                scores[e] = np.dot(weights[e], features)
            ...
            if predicted_emotion != emotion:
                # Increase weights for correct emotion using numpy operations
                weights[emotion] += learning_rate * features
                
                # Decrease weights for predicted emotion using numpy operations
                weights[predicted_emotion] -= learning_rate * features
            else:
                correct += 1
